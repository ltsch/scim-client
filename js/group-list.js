...
// SCIM Filter Builder for Groups
class SCIMFilterBuilder {
  ...
  buildFilterString() {
    ...
    if (this.searchTerm) {
      const searchFields = ['displayName', 'externalId'];
...
// Render filter UI for groups
function renderFilterUI(container, schema, onFilterChange) {
  const filterContainer = document.createElement('div');
  filterContainer.className = 'filter-container';
  
  const attributes = getSchemaAttributes(schema);
  const commonAttributes = ['displayName', 'externalId'];
  
  let html = `
    <div class="filter-section">
      <h3>Search & Filter Groups</h3>
      
      <!-- General Search -->
      <div class="form-group">
        <label for="group-search">General Search</label>
        <input type="text" id="group-search" class="form-control" 
               placeholder="Search across displayName, externalId..." />
        <small class="form-text">Searches across common group fields</small>
      </div>
  ...
  // Event listeners
  const searchInput = filterContainer.querySelector('#group-search');
...
export function renderGroupList(container, groups, onSelectGroup, options = {}) {
  ...
  if (!Array.isArray(groups) || groups.length === 0) {
    resultsInfo.innerHTML = '<div class="no-results">No groups found.</div>';
    return;
  }
  
  resultsInfo.innerHTML = `<div class="results-count">Showing ${groups.length} group(s)</div>`;
  ...
  groups.forEach((group, idx) => {
    html += `<tr data-group-id="${group.id}" class="summary-row">
      ...
      <td class="actions-cell">
        <button class="btn btn-secondary btn-sm edit-group-btn" data-group-id="${group.id}" data-group-index="${idx}">Edit</button>
        <button class="btn btn-danger btn-sm delete-group-btn" data-group-id="${group.id}" data-group-index="${idx}">Delete</button>
      </td>
    </tr>`;
  ...
  tableContainer.querySelectorAll('.edit-group-btn').forEach(btn => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const groupId = btn.getAttribute('data-group-id');
      const groupIndex = parseInt(btn.getAttribute('data-group-index'));
      const group = groups[groupIndex];
      if (group) onSelectGroup(group, 'edit');
    };
  });
  
  // Delete button handlers
  tableContainer.querySelectorAll('.delete-group-btn').forEach(btn => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const groupId = btn.getAttribute('data-group-id');
      const groupIndex = parseInt(btn.getAttribute('data-group-index'));
      const group = groups[groupIndex];
      if (group && confirm(`Are you sure you want to delete group "${group.displayName || group.id}"?`)) {
        onSelectGroup(group, 'delete');
      }
    };
  });
  ...
  tableContainer.querySelectorAll('.summary-row').forEach((row, idx) => {
    row.onclick = (e) => {
      if (e.target.classList.contains('expand-btn') || 
          ...
          e.target.classList.contains('delete-group-btn')) return;
      const group = groups[idx];
      if (group) onSelectGroup(group, 'edit');
    };
  });
